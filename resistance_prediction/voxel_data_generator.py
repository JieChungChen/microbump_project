...
class bump_augmentation(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            inputs = inputs.reshape(inputs.shape[0], 64, 64, 64)
            if mode == 1:
                angle = random.randint(5, 15)
                aug = rotate(inputs, axes=ax_tup, angle=angle, cval=0.0, reshape=False).reshape(inputs.shape[0], 64, 64, 64)
            elif mode == 2:
                aug = np.flip(inputs, axis).reshape(inputs.shape[0], 64, 64, 64)
            else:
                shift_lst = [0, 
                             math.floor(random.uniform(0.05, 0.15)*inputs.shape[0]), 
                             math.floor(random.uniform(0.05, 0.15)*inputs.shape[1]), 
                             math.floor(random.uniform(0.05, 0.15)*inputs.shape[2])]
                aug = shift(inputs, shift=shift_lst, cval=0).reshape(inputs.shape[0], 64, 64, 64)
            inputs = aug.copy()
        else:
...
class VoxelDataGenerator(object):
    ...
    def build(self,
        ...
        if(self.zoom_axis is not None) and (self.zoom_range is not None):
            ...
            if not (type(self.zoom_range) is str) and (type(self.zoom_range) in (int, float)):
                ...
                parse_dict['zoom_range'] = np.random.uniform(0.0, 0.2)
            else:
        ...
        if (self.rotate_axis is not None) and (self.rotate_angle is not None):
            ...
            if self.rotate_angle == 'random':
                parse_dict['rotate_angle'] = int(np.random.uniform(-15, 15))
    ...
    def _rotate_data(self, data, parse_dict):
        ...
        return rotate(data, axes=ax_tup, angle=parse_dict['rotate_angle'], cval=0.0, reshape=False).reshape(data.shape[0], 1, 64, 64, 64)
